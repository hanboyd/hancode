...
current_phase: Phase 4 step 6 closeout in flight at (pending commit SHA — to be filled in after step-6 commit). ADR-0014 accepted; version bump 0.4.0-candidate → 0.5.0-candidate in CMakeLists.txt / __version__ / pyproject.toml / test_bind_smoke.py; CHANGELOG [0.5.0-candidate] entry written with G1/G2/G3/G5 green table + G6 deferred row. 35/35 ctest Debug + 35/35 ctest Release after the version sync. Phase 4 closed.
...
next:
  ...
  - Do not re-run G3 (remotemic_audio_route_bind_smoke, remotemic_upsample_parity, remotemic_audio_route_parity, remotemic_phase4_native_switch, remotemic_phase4_production_routing, remotemic_bind_smoke version-sync) or full ctest unless a Phase 4 source file changes; (latest step 6 commit)'s 35/35 results are durable.
