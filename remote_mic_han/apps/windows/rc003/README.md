...
> 本候选已完成真实 RC003 的 BLE/ATVV 语音链路和 Windows 可见按键验收；
...
后续更新时直接运行新版本安装器，不需要先卸载或删除旧版本。安装器会停止
旧进程、替换程序文件，并沿用同一个 Windows 已安装应用条目；设备配置、按键
映射、使用统计和日志会保留。便携版 ZIP 不提供这种原位升级能力，需要无手工
清理的更新体验时应始终选择安装器。
