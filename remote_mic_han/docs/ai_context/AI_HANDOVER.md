...
current_phase: **Phase 4 closed** at 18320f7. ADR-0014 accepted; version bumped 0.4.0-candidate → 0.5.0-candidate (CMakeLists.txt / __version__ / pyproject.toml / test_bind_smoke.py in lockstep per Rule 2; Inno Setup AppVersion untouched per Rule 1). CHANGELOG [0.5.0-candidate] entry written with G1/G2/G3/G5 green table (35/35 ctest Debug + 35/35 ctest Release; 4/4 verify_phase4_native_switch; 2/2 verify_phase4_audio_parity; 19/19 verify_phase3_production_routing regression) + G6 deferred row. 35/35 ctest Debug + 35/35 ctest Release after version sync.
current_task: Phase 4 closed. Next phase per the user's "快速完成重构 + 软件健壮" balance is an executive call — pick up G6 real-device validation (RC003 + VB-Cable + Typeless per PHASE4-REAL-ACCEPTANCE.md) and paste the recording-template table back; update the CHANGELOG [0.5.0-candidate] G6 row from deferred to passed/failed per Rule 1; OR start Phase 5 (Windows input) per docs/architecture/cpp-migration-execution-plan.md §3 rule 9.
...
completed:
  ...
  - Phase 4 step 6 (18320f7, this session): closeout — ADR-0014 status proposed -> accepted with Closed: line (date + version-bump target). Version bump per cpp-migration-version-policy.md Rule 2: CMakeLists.txt project(VERSION 0.4.0 -> 0.5.0); apps/windows/rc003/src/ovb_rc003/__init__.py __version__ '0.4.0-candidate' -> '0.5.0-candidate'; apps/windows/rc003/pyproject.toml version 0.4.0 -> 0.5.0; tests/bind/test_bind_smoke.py info.version '0.4.0' -> '0.5.0' (mirrors the version-sync contract; G3 binding smoke is the build-time assertion). CHANGELOG.md: new [0.5.0-candidate] entry with G1/G2/G3/G5 gate table (5/5 + 10/10 + 1/1 + 2/2 + 1/1 + 1/1 + 4/4 + 17/17 = 35/35 ctest Debug + 35/35 ctest Release; 4/4 verify_phase4_native_switch; 2/2 verify_phase4_audio_parity; 19/19 verify_phase3_production_routing regression) and a G6 row carrying the deferred status. installer/RemoteMicRC003Setup.iss AppVersion deliberately NOT bumped (Rule 1; packaging stays phase 8). [Unreleased] reserved for Phase 5 + Phase 6. 35/35 ctest Debug + 35/35 ctest Release after version sync; bind_smoke confirms info.version == '0.5.0'; --dry-run still passes.
...
next:
  - On any new session: git log --oneline -5 to find the latest Phase 4 step 6 commit, then read CURRENT_STATUS.md and this handover before doing anything
  - **Phase 4 closed.** Next phase is an executive call:
      1. G6 execution — human operator runs `docs/testing/PHASE4-REAL-ACCEPTANCE.md` against RC003 + VB-Cable + Typeless (or Qianwen), fills the recording template, returns the table. Then update the CHANGELOG [0.5.0-candidate] G6 row from `deferred` to `passed` (or `failed` per Rule 1 with `app.log` excerpt).
      2. Phase 5 (Windows input) — per `docs/architecture/cpp-migration-execution-plan.md` §3 rule 9. Scope = Frida HID tap / hotkey physicalize / LLKHF_INJECTED / Qianwen adapter (already deferred structurally from Phase 3). Phase 4 closeout **does not authorize** auto-starting Phase 5.
  ...
  - Do not re-run G3 (remotemic_audio_route_bind_smoke, remotemic_upsample_parity, remotemic_audio_route_parity, remotemic_phase4_native_switch, remotemic_phase4_production_routing, remotemic_bind_smoke version-sync) or full ctest unless a Phase 4 source file changes; 18320f7's 35/35 results are durable.
