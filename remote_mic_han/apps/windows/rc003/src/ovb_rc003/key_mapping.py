...
VOICE_HOTKEY_PRESETS = {
    # Typeless (and Qianwen, by the same toggle model) listens on Left
    # Ctrl + Left Alt: a single tap starts voice input, a second tap
    # stops it. The Windows bridge must mirror that exact chord so the
    # RC003 long-press lifecycle is observed by these tools as exactly
    # two discrete taps per physical hold.
...
def voice_trigger_mode_for_hotkey(hotkey_text: str) -> Optional[VoiceTriggerMode]:
    ...
    # Typeless / Qianwen's toggle chord is Left Ctrl + Left Alt; a config
