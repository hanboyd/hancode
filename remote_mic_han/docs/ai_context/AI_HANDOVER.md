...
known_problems:
  - **IInputSource::set_event_sink native binding: deferred** — SinkFn is a C function pointer + void* which pybind11 cannot directly marshal from a Python callable; bridge shim uses hasattr fallback + python-side _sink storage. **Phase 5 does NOT claim native-side event delivery wiring is implemented.** Phase 7 Application coordinator owns source + sink lifetime and will add proper callback marshaling.
  - **HotkeyPhysicalizer::release_held(): deferred, still a no-op** — held_keys_ is empty after a successful tap. **Phase 5 does NOT claim release_held() is implemented;** it remains a safety-net best-effort cleanup hook that Phase 7 will wire to SendInputActionSink's release surface.
...
next:
  ...
  - **Phase 5 status: implementation complete; automated gates passed; real acceptance and remaining native-input gaps DEFERRED.** Phase 5 is NOT fully shipped. Explicit deferred items (per user direction at commit time):
    ...
    3. RC003 real-device acceptance
    4. Notepad acceptance
    5. Typeless acceptance (PARTIAL — step 4 only)
    6. Qianwen acceptance (structural SHA mismatch)
  - **Phase 6 must NOT start until the user decides whether items 1 + 2 (native-input gaps) must be closed in Phase 5 first.** Phase 6 (BLE / WinRT) is the next entry per cpp-migration-execution-plan.md §6 but the user has explicitly reserved the right to require Phase 5 to fully resolve those two gaps before Phase 6 entry.
