...
def default_config() -> Dict[str, Any]:
    ...
    return {
        ...
        # Typeless (and Qianwen, by the same short-press model) listens on
        # Left Ctrl + Left Alt. Keep the default aligned so a fresh install
        # already speaks the host chord these tools actually react to; the
        # ``voice_hotkey`` field on disk overrides this per user.
